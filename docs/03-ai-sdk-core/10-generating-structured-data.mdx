...
The Go AI SDK standardizes structured object generation across model providers with the [`ai.GenerateObject()`](../07-reference/ai/generate-object.md) and [`ai.StreamObject()`](../07-reference/ai/stream-object.md) functions. You can use both functions with different output strategies, e.g. `array`, `object`, `enum`, or `no-schema`, and with different generation modes, e.g. `auto`, `tool`, or `json`.

You can use JSON schemas to specify the shape of the data that you want, and the AI model will generate data that conforms to that structure.
