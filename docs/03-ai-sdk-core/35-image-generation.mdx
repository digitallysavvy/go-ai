...
func main() {
    ...
    // Save image to file
    err = os.WriteFile("generated.png", result.Image, 0644)
    if err != nil {
        log.Fatal(err)
    }
...
// As base64 string
base64String := result.Base64Image
...
saveImage(result.Image, result.MimeType, "sunset")
```
...
```go
func generateVariations(ctx context.Context, model provider.ImageModel, prompt string, count int) ([][]byte, error) {
    result, err := ai.GenerateImage(ctx, ai.GenerateImageOptions{
        Model:  model,
        Prompt: prompt,
        N:      count,
...
for i, img := range images {
    os.WriteFile(fmt.Sprintf("variation-%d.png", i+1), img, 0644)
}
```
...
```go
func generateImageWithRetry(ctx context.Context, model provider.ImageModel, prompt string, maxRetries int) (*types.ImageResult, error) {
    var lastErr error
