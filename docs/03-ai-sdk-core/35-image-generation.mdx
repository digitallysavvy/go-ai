...
The xAI provider returns all generated images in `result.Images`. When xAI returns `b64_json`, the original provider base64 strings are also available in `result.Base64Images`.
