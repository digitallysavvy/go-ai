...
MCP JSON parsing uses secure parsing to avoid prototype-pollution style payloads when consuming remote server data. MCP clients expose server instructions and propagate the server name through dynamic tool parts so tool calls and telemetry can identify the originating server.
