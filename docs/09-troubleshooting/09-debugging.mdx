...
func (m *MockLanguageModel) DoStream(ctx context.Context, options *provider.GenerateOptions) (provider.TextStream, error) {
    if m.ShouldError {
        return nil, m.ErrorToReturn
    }

    chunks := make([]provider.StreamChunk, 0, len(m.Response)+1)
