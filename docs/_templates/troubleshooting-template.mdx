...
func main() {
    ...
    client := newExampleClient(
        apiKey,
        exampleWithOption1(value1),
        exampleWithOption2(value2),
    )
...
client := newExampleClient(
    apiKey,
    exampleWithHTTPClient(httpClient),
)
```
...
func generateWithRetry(request ai.GenerateTextOptions) (*ai.GenerateTextResult, error) {
    maxRetries := 3
    backoff := time.Second
...
func validateRequest(request ai.GenerateTextOptions) error {
    if request.Model == "" {
...
```go
request.MaxTokens = ptr(500) // Reduce for faster responses
```
...
```go
request.MaxTokens = ptr(1000) // Prevent extremely long responses
```
...
func main() {
    ...
    client := newExampleClient("test-key")
...
- [Community Examples](https://github.com/digitallysavvy/go-ai/tree/main/examples)
