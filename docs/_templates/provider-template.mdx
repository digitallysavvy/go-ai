...
import (
    ...
    "github.com/digitallysavvy/go-ai/pkg/providers/[provider-package]"
)
...
func main() {
    ...
    client := newExampleClient(
        "your-api-key",
        exampleWithProvider([provider-package].NewProvider()),
    )
...
client := newExampleClient(apiKey, exampleWithProvider(provider))
```
...
request := ai.GenerateTextOptions{
    Model: "[model-name]",
    Messages: messages,
    Temperature: ptr(0.7), // Balanced creativity
}
```
...
request := ai.GenerateTextOptions{
    Model: "[model-name]",
    Messages: messages,
    MaxTokens: ptr(1000), // Limit to ~750 words
}
```
...
request := ai.GenerateTextOptions{
    Model: "[model-name]",
    Messages: messages,
    TopP: ptr(0.9), // Consider top 90% probability mass
}
```
...
func main() {
    ...
    stream, err := ai.StreamText(context.Background(), request)
    if err != nil {
...
```go
embedding, err := ai.Embed(context.Background(), ai.EmbedOptions{
