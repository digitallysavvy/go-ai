...
OpenAI-compatible providers use camelCase provider option keys, matching the TypeScript SDK public API. Deprecated kebab-case or snake_case keys are still accepted for migration compatibility, but they emit warnings so callers can move to camelCase.
...
result, err := ai.GenerateText(ctx, ai.GenerateTextOptions{
    ...
    ProviderOptions: map[string]interface{}{
        "groq": map[string]interface{}{
            "reasoningEffort": "high", // preferred
