...
7. Run `go test ./...` for packages. For examples that use `//go:build ignore`, compile the specific example file you changed, for example `go build ./examples/upload-file/provider-reference/main.go`.
...
- MCP: secure JSON parsing, server instructions, TypeScript-compatible MCP provider metadata, MCP Apps helpers, negotiated protocol headers, and custom transports.
