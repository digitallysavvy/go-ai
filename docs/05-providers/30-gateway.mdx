...
`gateway.GatewayProviderOptions` includes `Sort`, `QuotaEntityID`, `HIPAACompliant`, `DisallowPromptTraining`, `ZeroDataRetention`, provider order, model filters, and BYOK options.
