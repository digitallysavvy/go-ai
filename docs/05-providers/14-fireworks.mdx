...
Streaming language-model requests include `stream_options.include_usage: true`, so final stream usage is available when Fireworks returns it.
...
func main() {
    ...
    result, err := ai.GenerateText(ctx, ai.GenerateTextOptions{Model: model, Prompt: "Explain Fireworks AI"})
    if err != nil {
        log.Fatal(err)
    }
