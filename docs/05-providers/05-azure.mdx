...
func (ct *CostTracker) Track(model string, usage ai.Usage) {
    ...
    currentInput := current.GetInputTokens()
    currentOutput := current.GetOutputTokens()

    newInput := currentInput + inputTokens
    newOutput := currentOutput + outputTokens
    current.InputTokens = &newInput
    current.OutputTokens = &newOutput
