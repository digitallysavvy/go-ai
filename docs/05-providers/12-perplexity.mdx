...
result, err := ai.GenerateText(ctx, ai.GenerateTextOptions{
    Model:  model,
    Prompt: "Show me Mars rovers",
})

if metadata, ok := result.ProviderMetadata["perplexity"].(perplexity.PerplexityMetadata); ok {
...
func main() {
    ...
    result, err := ai.GenerateText(ctx, ai.GenerateTextOptions{
        Model:  model,
        Prompt: "What are today's top technology news?",
    })
    if err != nil {
        log.Fatal(err)
    }
...
```go
func researchTopic(ctx context.Context, model provider.LanguageModel, topic string) {
    result, err := ai.GenerateText(ctx, ai.GenerateTextOptions{
        Model:  model,
        Prompt: fmt.Sprintf("Research and summarize: %s", topic),
    })
    if err != nil {
        log.Fatal(err)
    }
