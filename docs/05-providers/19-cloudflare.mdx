...
```go
// In Workers code
result, err := ai.GenerateText(ctx, ai.GenerateTextOptions{Model: model, Prompt: userPrompt})
// No egress charges
```
...
func main() {
    ...
    result, err := ai.GenerateText(ctx, ai.GenerateTextOptions{Model: model, Prompt: "Explain edge computing"})
