...
The default base URL is `https://aws-external-anthropic.{region}.api.aws`. `WorkspaceID` is required and is sent as `anthropic-workspace-id`.
...
|----------|---------|-------------|--------------|----------|
| claude-opus-4-7 | 200K | TBA | TBA | Latest Opus reasoning and coding |
| claude-opus-4-6 | 200K | TBA | TBA | Complex tasks, fast mode |
| claude-sonnet-4-6 | 200K | TBA | TBA | Balanced performance and capability |
...
result, err := ai.GenerateText(model, longConversation...)
...
(Headers are automatically added by the SDK)
