...
Vertex-specific `serviceTier` values are formatted for Vertex API requests when shared Google request code is used; use the `"google-vertex"` provider key for Vertex options and `"google"` for AI Studio options.
