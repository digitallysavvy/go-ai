...
| StopWhen | []ai.StopCondition | No | Stable multi-step stop conditions, for example `[]ai.StopCondition{ai.IsStepCount(10)}` |
| MaxSteps | int | No | Deprecated compatibility shorthand for `StopWhen: []ai.StopCondition{ai.IsStepCount(n)}` |
