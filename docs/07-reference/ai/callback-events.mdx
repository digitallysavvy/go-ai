...
Fired after each tool completes (success or failure). Use this event with `OnToolExecutionEnd`. This is a discriminated union: either `Result` or `Error` is non-nil.
