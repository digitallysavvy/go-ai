...
When a function tool is serialized for provider requests, nil parameters or a
schema with `properties` but no `type` are normalized to an explicit object
schema. For example, an empty tool schema is sent as
`{"type":"object","properties":{}}`, matching the TypeScript SDK request shape.
