...
func makeTestVertexProvider() *Provider {
	p, _ := New(Config{
...
func TestVertexReasoningNoneDisablesThinking(t *testing.T) {
	prov := makeTestVertexProvider()
...
func TestVertexReasoningDefaultOmitsThinking(t *testing.T) {
	prov := makeTestVertexProvider()
	model := NewLanguageModel(prov, "gemini-2.5-pro")
...
func TestVertexReasoningNilOmitsThinking(t *testing.T) {
	prov := makeTestVertexProvider()
	model := NewLanguageModel(prov, "gemini-2.5-pro")
...
func TestVertexReasoningDynamicBudget(t *testing.T) {
	prov := makeTestVertexProvider()
...
func TestVertexReasoningMinIsAtLeastOne(t *testing.T) {
	prov := makeTestVertexProvider()
...
func TestMaxThinkingTokensForVertexModel(t *testing.T) {
	tests := []struct {
		modelID string
		want    int
	}{
	...
	for _, tt := range tests {
		t.Run(tt.modelID, func(t *testing.T) {
			got := maxThinkingTokensForVertexModel(tt.modelID)
			if got != tt.want {
				t.Errorf("maxThinkingTokensForVertexModel(%q) = %d, want %d", tt.modelID, got, tt.want)
			}
		})
	}
}
